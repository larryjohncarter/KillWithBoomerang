...
namespace FTemplateNamespace
{
	public class MenuItems
	{
		...
		[MenuItem( "Flamingo/Generate Lightmap", priority = 50 )]
		private static void GenerateLightmap()
		{
			Lightmapping.BakeAsync();
		}

		[MenuItem( "Flamingo/Disable Unused Renderer Properties", priority = 51 )]
		private static void DisableUnusedRendererProperties()
		{
			GameObject[] roots = SceneManager.GetActiveScene().GetRootGameObjects();
			for( int i = 0; i < roots.Length; i++ )
				DisableUnusedRendererPropertiesRecursive( roots[i].transform );

			EditorSceneManager.MarkAllScenesDirty();
		}

		private static void DisableUnusedRendererPropertiesRecursive( Transform obj )
		{
			for( int i = obj.childCount - 1; i >= 0; i-- )
				DisableUnusedRendererPropertiesRecursive( obj.GetChild( i ) );
