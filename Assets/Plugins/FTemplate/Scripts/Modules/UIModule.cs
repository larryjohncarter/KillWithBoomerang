...
namespace FTemplateNamespace
{
	public class UIModule : MonoBehaviour
	{
		...
		public class FollowTransformJob : IAnimationJob
		{
			private Transform transform, target;
			private Vector3 positionOffset;
			private float t;

			public void Initialize( Transform transform, Transform target, Vector3 positionOffset, float duration )
			{
			...
			public bool Execute( float deltaTime )
			{
				...
				if( t > 0f )
				{
					transform.SetPositionAndRotation( target.TransformPoint( positionOffset ), target.rotation );
					return true;
				}
...
#pragma warning disable 0649
		...
		[SerializeField]
		private Animation totalCoinsTextAnim;
		...
		[SerializeField]
		private Animation settingsPanel;
		...
		[SerializeField]
		private Animation nextUnlockUnlockedAnim;
		...
		[SerializeField]
		private Animation levelFailedRestartOnlyModeAnim;
		[SerializeField]
		...
		[SerializeField]
		private ParticleSystem bonusLevelTextParticles;
...
#pragma warning restore 0649
		...
		public float DisplayedFeverProgress { get; private set; }
		...
		private void Awake()
		{
			...
			if( settingsButton )
			{
				settingsButton.onClick.AddListener( () =>
				{
					...
					settingsPanel.Play( settingsPanelVisible ? "SettingsPanelShow" : "SettingsPanelHide" );
...
#pragma warning disable 0618
				// Fixes SendMouseEvents GC that appears in Profiler
				if( !CameraTransform.GetComponent<GUILayer>() )
					CameraTransform.gameObject.AddComponent<GUILayer>();
#pragma warning restore 0618
		...
		public void Show( UIElementType target, float duration = UI_ELEMENTS_DEFAULT_TOGGLE_DURATION, float delay = 0f, float autoHideInSeconds = 0f )
		{
			...
			{
				// HUD Elements
				case UIElementType.BonusLevelText:
					Vector2 bonusLevelTextPosition = RectTransformUtility.WorldToScreenPoint( null, uiElement.NavigationHandler.transform.position );
					Vector3 particlesOffset = CameraTransform.InverseTransformPoint( Camera.ScreenToWorldPoint( new Vector3( bonusLevelTextPosition.x, bonusLevelTextPosition.y, 1.5f ) ) );

					float particlesScale = Camera.orthographic ? ( Camera.orthographicSize * 0.225f ) : ( Camera.fieldOfView * 0.00325f );
					bonusLevelTextParticles.transform.localScale = new Vector3( particlesScale, particlesScale, particlesScale );

					var mainModule = bonusLevelTextParticles.main;
					mainModule.gravityModifierMultiplier = 2.3f * particlesScale;

					AnimationSystem<FollowTransformJob>.NewAnimation().Initialize( bonusLevelTextParticles.transform, CameraTransform, particlesOffset, 5f );
					bonusLevelTextParticles.gameObject.SetActive( true );
					bonusLevelTextParticles.Play( false );
				...
				case UIElementType.MainMenu:
					...
					if( settingsButton )
					{
						...
						settingsPanel.Play( "SettingsPanelHide" );
						settingsPanel["SettingsPanelHide"].normalizedTime = 1f;
					}
				...
				case UIElementType.LevelFailedMenu:
					...
					if( LevelFailedMenuType == LevelFailedMenu.RestartOnly )
						levelFailedRestartOnlyModeAnim.Play();
					else
					{
						if( topRestartLevelButton )
		...
		public void Hide( UIElementType target, float duration = UI_ELEMENTS_DEFAULT_TOGGLE_DURATION, float delay = 0f )
		{
			...
			switch( target )
			{
				// HUD Elements
				case UIElementType.BonusLevelText:
					bonusLevelTextParticles.gameObject.SetActive( false );
					break;
		...
		public void HideAllLoadingBars( float duration = UI_ELEMENTS_DEFAULT_TOGGLE_DURATION )
		{
			for( int i = 0; i < UIElement.UI_LOADING_BARS.Length; i++ )
				Hide( UIElement.UI_LOADING_BARS[i], duration );
		}
		...
		{
			if( DisplayedFeverProgress != progress )
			{
				DisplayedFeverProgress = progress;
				feverSlider.fillAmount = progress;
			}
		}
